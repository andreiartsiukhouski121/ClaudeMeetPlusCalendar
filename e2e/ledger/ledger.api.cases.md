...
- **Две отвергнутые формулировки этого правила**, обе поймал прогон: «не больше одного `pending`»
  сломалось на первом коммите, вносившем изменение процесса и дефект одновременно; «на `HEAD~1`
  записей `pending` нет» — off-by-one, потому что `HEAD~1` это и есть коммит, где `pending`
  законен. Записаны здесь, чтобы к ним не вернулись.
