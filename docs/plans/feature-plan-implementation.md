...
> **АРХИВ. Ссылаться на этот документ за конвенцией нельзя.**
> Он описывает, как были сделаны две первые фичи, и сохранён ради «почему», а не ради «как
> сейчас». Живая конвенция сьюта — [`e2e/README.md`](../../e2e/README.md), инварианты —
> [`CLAUDE.md`](../../CLAUDE.md), порядок приёмки — скил `regression-verify`. Числа в этом файле
> устарели (`FX-013`, `FX-027`).

Парный документ — [feature-plan-testing.md](feature-plan-testing.md) (организация тестов и тест-кейсы).
Разделение ответственности на момент первой итерации: этот файл описывал архитектуру и порядок
задач, тест-план — структуру сьюта. Сегодня оба вопроса закрыты живыми документами, перечисленными
в шапке.
