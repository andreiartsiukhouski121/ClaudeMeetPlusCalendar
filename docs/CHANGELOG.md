...
| ID     | Дата       | Что                                                                                                                 | Коммит    |
| ------ | ---------- | ------------------------------------------------------------------------------------------------------------------- | --------- |
| CH-008 | 2026-09-08 | CI на GitHub Actions: job `verify` по шагам + production-сборка с артефактом; локальный `verify` выровнен с CI      | `pending` |
...
| ------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | --------- |
| FX-019 | 2026-09-08 | Инвариант `pending` делал **первый push любой новой записи красным по построению**: на CI дерево всегда чистое, а хеш заполняется следующим коммитом | первый прогон CI                               | `pending` |
| FX-018 | 2026-09-08 | `LG-API-04` объявлял весь реестр битым на CI: `actions/checkout` делает shallow clone, и `git cat-file` не находит ни одного старого коммита         | первый прогон CI                               | `pending` |
