...
письменных ответа про дубль, конфликт с реализованным, конфликт с планируемым и неясности. Пустой
ответ означает, что ориентацию пропустили. Если задача оказалась дублем — сказать это и
остановиться: «уже сделано в `FX-007`» — полноценный результат, а не отказ от работы.
