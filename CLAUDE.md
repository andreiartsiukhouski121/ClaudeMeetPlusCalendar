...
Двум агентам, работающим одновременно, выдавай **свои диапазоны портов** — иначе они столкнутся на
3100/3101 и получат чужие результаты:

```bash
E2E_WEB_PORT=3200 E2E_API_PORT=3201 pnpm e2e     # второй агент
```

Переменные читает `playwright.config.ts`, и `API_URL` для web-сервера считается из них же
(проверено прогоном). Для полной изоляции рабочего дерева — отдельный git worktree на агента.

## Инварианты проекта

Пятнадцать правил, на которых уже ломались реализации. Каждое стоило отдельного разбора, поэтому
