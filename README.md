...
`docs/plans/TEMPLATE.md`, одно ревью, реализация — каждый агент в своём git worktree). Приёмка фичи — по скилу `regression-verify` (`.claude/skills/`): обязательный прогон юнитов, API- и
